...
export const httpAddOneProductToCart = async (req, res) => {
    try {
        ...
        const updatedCart = await cartInstance.AddOneProductToCart(cid, { _id: productId, quantity });

        return res.status(200).json({ message: "Producto agregado al carrito", payload: updatedCart });
    } catch (error) {
